...
const slice = createSlice({
    ...
    reducers: {
        ...
        changeTodolistFilter(
            state,
            action: PayloadAction<{ filter: FilterType; id: string }>,
        ) {
            const index = state.findIndex((t) => t.id === action.payload.id)
...
}>('todoList/fetchTodoList', async (arg, thunkAPI) => {
    let todoLists
    try {
...
const createTodoListTC = createAppAsyncThunk<TodoListDomainType, string>(
...
const updateTodoListTitleTC = createAppAsyncThunk<
    ...
    UpdateTodoListArgType
>('todoList/updateTodoListTitle', async (arg, thunkAPI) => {
